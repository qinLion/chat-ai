...
public class WebSocketService {
    ...
    public void onMessage(String message, Session session) {
        ...
        // chatMessageQuery.setContent(message);
        ...
        logger.info("对于用户id:{},提出的问题是：{}，回答的问题是{}", chatMessageQuery.getToken(), chatMessageQuery.getContent(), chat);
        // 4、将返回的数据进行存储
        if (!chat.equals("")) {
        ...
        // 5、不管是用户主动断开还是发送完毕的断开，消息发送完毕都要断开连接
        onClose(session);
        sessionToToken.remove(session);
        CLIENTS.remove(chatMessageQuery.getToken());
    }
